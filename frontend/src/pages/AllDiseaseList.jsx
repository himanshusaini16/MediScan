...
const AllDiseaseList = () => {
  ...
  return (
    <div className="p-8 bg-gradient-to-br from-blue-50 to-white min-h-screen">
      <h2 className="text-4xl font-bold text-center text-blue-800 mb-12">🩺 Choose a Disease to Predict</h2>
      <div className="'w-full grid  grid-cols-5 gap-5 gap-y-10 ">
        {diseases.map((disease, index) => (
          <div
            key={index}
            className='border w-50 h-50  border-blue-200 bg-blue-200 bg rounded-xl overflow-hidden cursor-pointer  transition-all duration-500'
          >
              <img
                src={disease.image}
                alt={disease.name}
            ...
            <h3 className="text-xl font-semibold text-blue-900 m-2 text-center">{disease.name}</h3>
            <p className="text-gray-500 mb-6 text-sm text-center text-gray-600">{disease.description}</p>
            <button
