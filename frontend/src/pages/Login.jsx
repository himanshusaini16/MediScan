...
const Login = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      if(state === 'Sign Up'){
        const {data} = await axios.post(backendUrl + '/api/user/register',{name,password: pass,email})
        if(data.success){
  ...
  return (
    <form onSubmit={onSubmitHandler} className="min-h-[80vh] flex items-center">
      <div className="flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 tetx-sm shadow-lg">
        ...
        <p>
          Please {state === "Sign Up" ? "sign in" : "Log in"} to predict disease & book 
          appointment.
        </p>
        {
        ...
        {state === "Sign Up" ? (
          <p className="">Already have an account? <span onClick={()=>setState("Login")} className="text-primary underline cursor-pointer">Login here</span><br/><a href="http://localhost:5174/" className="text-primary underline cursor-pointer">Click Here For Admin or Doctor Login</a></p>
        ) : (
